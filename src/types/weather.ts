...
export interface SafeFetchError {
  kind: 'network' | 'http' | 'invalid-content-type' | 'invalid-json'
  message: string
  status?: number
  contentType?: string
  bodyPreview?: string
...
export interface WeatherAlert {
  ...
  geometryStatus: AlertGeometryStatus
  sourceUrl: string
}
...
export interface RainViewerRadarState {
  version: string | null
...
export interface SpcReportsState {
  reports: SpcStormReport[]
  byType: Record<SpcReportType, number>
  sourceUrl: string
  fetchedAt: string
  error?: SafeFetchError | { kind: 'parse'; message: string; bodyPreview?: string }
}

export interface SpcOutlookState {
  sourceUrl: string
  fetchedAt: string
